...
public class BuildLayer implements TabExecutor {
    ...
    public int spacing() {return config.getInt("layers.spacing");}
    public double blockFrequency() {return config.getDouble("layers.blockFrequency");}
    public double blockIncrement() {return (1-blockFrequency())/4.0;}
    public double circleAdjustment() {return config.getDouble("layers.circleAdjustment");}
    ...
    public Material[][][] makeLayer(int arenaSize, boolean special) {
        ...
        for(int x = 0; x<arenaSize; x++) {
            for(int z = 0; z<arenaSize; z++) {
                if(dist(x+0.5-arenaSize/2.0,z+0.5-arenaSize/2.0)>arenaSize/2.0+ circleAdjustment()) {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        for(int n = 0; n<numLayers; n++) {
            Material[][][] layer = makeLayer(Math.max(endX-startX,endZ-startZ),special);
